plotresults<-function(dat,maintitle="Expt 1 \n (critical region)",ylabel="Estimates (ms)",
                      cols=c("darkgray", "black","black"),removelegend=FALSE,legendposition=c(0.4,0.8),lowery=-100,uppery=250,xaxisblank=FALSE){
  ...
  if(removelegend){
    ...
    resultsplot+ scale_color_manual(values=cols)+ scale_shape_manual(values=c(0,1,2,4)) +theme(legend.position=legendposition,legend.title = element_blank())
  }
}
